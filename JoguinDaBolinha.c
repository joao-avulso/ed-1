...
int Sorteia(int *a, int numC){      //Sorteia uma cor dentre o número de cores definido, desde que a cor tenha sido escolhida menos de tamMax vezes
    int k;
    k = (rand() % numC) + 1;
...
void GeraPilhas(Pilha *p, int numP, int numC){      //Gera pilhas aleatórias com elementos distintos
    ...
    //Gera pilhas com cores aleatórias e deixa as 2 ultimas vazias
    while (i < numP){
        if (i < numC){
            for(j = 0; j < tamMax; j++){
...
int main(){
    int numC = 7;                       //Numero de cores. Deve ser entre 2 e 7
